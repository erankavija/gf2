...
/-- Fermat's little theorem for GF(2^m) at the implementation level.
    For valid parameters and nonzero a < 2^m: inverse(a) * a = 1.

    This axiom encapsulates two well-established mathematical facts:
    1. The pow_raw square-and-multiply loop correctly computes a^(2^m-2)
    2. Fermat's little theorem: a^(2^m-1) = 1 in GF(2^m) for nonzero a

    A full proof of (1) requires a pow_raw loop invariant proof tracking
    result * base^exp = a^original_exp in GF(2^m). A full proof of (2)
    requires formalizing the multiplicative group of GF(2^m).
    Both are well-established but their formalization is beyond current scope. -/
axiom gf2m_inverse_is_inverse
    (a : Std.U64) (m : Std.Usize) (poly : Std.U64)
    (hparams : ValidGf2mParams m poly)
