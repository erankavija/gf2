...
fn test_dvb_t2_polynomials_match_database() {
    ...
    for &frame_size in &frame_sizes {
        for &rate in &rates {
            ...
            let result = PrimitivePolynomialDatabase::verify(params.field_m, params.primitive_poly);
...
fn test_all_dvb_t2_short_frames_use_same_field() {
    ...
    for &rate in &rates {
        ...
        assert_eq!(params.field_m, expected_m, "All short frames use GF(2^14)");
        assert_eq!(
            params.primitive_poly, expected_poly,
...
fn test_all_dvb_t2_normal_frames_use_same_field() {
    ...
    for &rate in &rates {
        ...
        assert_eq!(params.field_m, expected_m, "All normal frames use GF(2^16)");
        assert_eq!(
            params.primitive_poly, expected_poly,
