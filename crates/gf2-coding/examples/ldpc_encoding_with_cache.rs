...
fn main() {
    ...
    println!("   Time: {:.2}s (preprocessing H matrix)", time_no_cache.as_secs_f64());
    println!("   Parameters: n={}, k={}\n", encoder_no_cache.n(), encoder_no_cache.k());
    ...
    let start = Instant::now();
    let encoder_cached_1 = LdpcEncoder::with_cache(code.clone(), &cache);
    let time_first = start.elapsed();
    
    println!("   Time: {:.2}s (similar to no-cache)", time_first.as_secs_f64());
    ...
    let start = Instant::now();
    let encoder_cached_2 = LdpcEncoder::with_cache(code.clone(), &cache);
    let time_second = start.elapsed();
    
    println!("   Time: {:.6}s (<1μs - instant!)", time_second.as_secs_f64());
    ...
    println!("   Encoded {} bits → {} bits", message.len(), codeword.len());
    println!("   Encoding time: {:.3}ms (same for all)\n", encode_time.as_secs_f64() * 1000.0);
    ...
    println!("   Precomputation time: {:.1}s (one-time cost)", precomp_time.as_secs_f64());
    ...
    for rate in &rates {
        let start = Instant::now();
        let _enc = LdpcEncoder::with_cache(
