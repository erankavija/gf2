...
fn main() {
    ...
    let cache_dir = Path::new("target/ldpc_cache");
    
    match mode {
        "generate" => generate_cache(cache_dir),
        "use" => use_cache(cache_dir),
        _ => {
            eprintln!("Usage: {} [generate|use]", args[0]);
            std::process::exit(1);
        }
    }
}
...
fn generate_cache(cache_dir: &Path) {
    ...
    match EncodingCache::precompute_and_save_dvb_t2(cache_dir) {
        Ok(()) => {
            ...
            println!("\n✓ Cache generation complete in {:.1}s", elapsed.as_secs_f64());
            println!("✓ 12 DVB-T2 configurations saved to {}", cache_dir.display());
            ...
            if let Ok(entries) = std::fs::read_dir(cache_dir) {
                ...
                println!("✓ Total cache size: {:.1} MB ({} files)", 
                         total_size as f64 / 1_000_000.0, count);
            }
...
fn use_cache(cache_dir: &Path) {
    ...
    for (name, rate, is_short) in configs {
        ...
        println!("  DVB-T2 {} (n={}, k={}): {:?}", 
                 name, encoder.n(), encoder.k(), create_time);
    ...
    println!("✓ Encoded {} bits → {} bits in {:?}",
             encoder.k(), encoder.n(), encode_time);
