...
// F_7 LUT init guard
//
// Ensures `permanent_bipedal7_hip_init` is called exactly once before any
// batch kernel launch. The Once is process-global; re-entrant calls from
// multiple threads block until init completes.
// ---------------------------------------------------------------------------

static GF7_INIT: Once = Once::new();
...
///
/// Called automatically by [`compute_permanent_gf7`] and
/// [`compute_permanent_gf7_batch`] through a process-global [`Once`] guard.
/// Callers that need the LUT available before the first compute call (e.g., for
/// the criterion-3 checksum test) should call this function directly.
///
...
pub unsafe fn init_permanent_gf7(
    ...
    // SAFETY: preconditions forwarded verbatim from the caller (see doc comment).
    unsafe { permanent_bipedal7_hip_init(host_add_lut, host_sub_lut, host_mul_lut) }
}

/// Compute the F_7 permanent of a single n×n matrix on the GPU.
///
/// Delegates to [`compute_permanent_gf7_batch`] with `m = 1`. The F_7 LUTs are
/// initialised on first call via a process-global [`Once`] guard (using the host
/// static consts from `gf2_algebra::packed::packed7`).
///
/// # Arguments
///
/// - `matrix_ptr` — device pointer to an `n × n` row-major array of `u8`
...
/// folding). Host overhead is one `Once`-guarded init (first call only) plus a
...
/// `hip/permanent/permanent_bipedal7.hip`. On first call, initialises the device
/// LUTs from the host static consts via a process-global [`Once`] guard. Launches
/// one HIP block per matrix (grid = M, block = 1); only thread 0 per block
/// executes the Gray walk. GPU throughput derives from many blocks running
/// simultaneously.
///
/// # LUT placement
///
...
pub unsafe fn compute_permanent_gf7_batch(
    ...
    // Ensure LUTs are copied to device memory exactly once per process.
    // SAFETY: the closure runs at most once and all pointer accesses are to
    // 'static data from gf2_algebra::packed::packed7.
    GF7_INIT.call_once(|| {
        use gf2_algebra::packed::packed7::{ADD_LUT, MUL_LUT, SUB_LUT};
        // SAFETY: ADD_LUT, SUB_LUT, MUL_LUT are 'static [u8; 65536].
        // permanent_bipedal7_hip_init only reads the pointed-to bytes.
        let rc = unsafe {
            permanent_bipedal7_hip_init(ADD_LUT.as_ptr(), SUB_LUT.as_ptr(), MUL_LUT.as_ptr())
        };
        // If init fails the GPU LUTs are unpopulated; subsequent kernel launches
        // will read zeros. The error is not surfaced here (Once closures cannot
        // return errors); callers that need early failure detection should call
        // `init_permanent_gf7` directly before their first batch call.
        let _ = rc;
    });
