...
mod tests {
    ...
    fn test_batch_matvec_single_vector() {
        ...
        let results = backend.batch_matvec(&identity, &[vec.clone()]);
        assert_eq!(results.len(), 1, "Single vector should return single result");
    ...
    fn test_batch_matvec_correctness() {
        ...
        let vectors: Vec<BitVec> = (0..5)
            .map(|_| BitVec::random(8, &mut rng))
            .collect();
        ...
        for (i, vec) in vectors.iter().enumerate() {
            ...
            assert_eq!(
                ...
                "Batch result at index {} should match individual computation", i
            );
        }
    }
    ...
    fn test_batch_matvec_transpose_correctness() {
        ...
        let vectors: Vec<BitVec> = (0..3)
            .map(|_| BitVec::random(10, &mut rng))
            .collect();
        ...
        for (i, vec) in vectors.iter().enumerate() {
            ...
            assert_eq!(
                batch_results[i], individual_result,
                "Batch transpose result at index {} should match individual", i
            );
        }
    }
    ...
    fn test_batch_matvec_parallel_speedup() {
        ...
        let vectors: Vec<BitVec> = (0..100)
            .map(|_| BitVec::random(1000, &mut rng))
            .collect();
        ...
        let start = Instant::now();
        let seq_results: Vec<_> = vectors.iter()
            .map(|v| backend.matvec(&matrix, v))
            .collect();
        let seq_time = start.elapsed();
        
        // Results should match
        assert_eq!(batch_results, seq_results, "Parallel results should match sequential");
        
        // Parallel should be faster (at least 1.5x on multi-core)
        // Note: This is a soft check; actual speedup depends on CPU cores
        println!("Batch time: {:?}, Sequential time: {:?}", batch_time, seq_time);
        println!("Speedup: {:.2}x", seq_time.as_secs_f64() / batch_time.as_secs_f64());
    }
