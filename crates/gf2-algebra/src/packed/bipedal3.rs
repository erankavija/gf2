...
//! - **sub**: 7 word-level ops (`neg(b)` costs 1 extra XOR).
...
impl PackedField<Fp<3>> for Bipedal3 {
    ...
    ///
    /// Implemented as `add(self, neg(rhs))` (7 ops total: 1 for neg, 6
    /// for add).
    ///
    /// # Arguments
    ///
    ...
    /// # Complexity
    ///
    /// `O(1)`: 7 word-level bitwise operations.
    #[inline]
    fn sub(self, rhs: Self) -> Self {
        let am = self.mag;
        let asg = self.sgn;
        let bm = rhs.mag;
        // neg(b): mag stays, sgn XORed with mag.
        let bsg = rhs.sgn ^ bm;
        // Then add(a, neg(b)).
        let t = am ^ asg ^ bsg;
        let u = bm & t;
