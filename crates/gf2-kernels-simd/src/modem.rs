...
mod tests {
    ...
    /// Deterministic LCG to keep parity tests self-contained (mirrors
    /// the pattern used in `gf2-coding` so parity assertions are
    /// reproducible without pulling in an RNG dev-dependency).
    struct Lcg(u64);
    impl Lcg {
        ...
        fn next_positive_f32(&mut self, lo: f32, hi: f32) -> f32 {
            let v = (self.next_u64() >> 32) as u32;
        ...
        fn next_positive_f64(&mut self, lo: f64, hi: f64) -> f64 {
            let v = (self.next_u64() >> 32) as u32;
    ...
    fn run_parity_f32(axis_len: usize, num_symbols: usize, seed: u64) {
        ...
        for s in 0..num_symbols {
            z.push(rng.next_signed_f32() * 2.5);
    ...
    fn run_parity_f64(axis_len: usize, num_symbols: usize, seed: u64) {
        ...
        let mut inv_n0 = Vec::with_capacity(num_symbols);
        for s in 0..num_symbols {
            z.push(rng.next_signed_f64() * 2.5);
