...
The CPU-only/parallel contract is regression-guarded by two complementary slow-tier integration suites: `tests/parallel_determinism.rs` (the direct `frame_sim` dispatch, issue `3fcb7025`) and `tests/determinism.rs` (the typestate preset production path via `Pipeline::dvb_t2()` + heartbeat-resume parity, issue `48a0db6c`); both share the four-column / BER-excluded comparison in `tests/common/mod.rs`. The CPU-vs-GPU relaxed contract (the three columns `fer`/`frames`/`errors` byte-identical, `mean_iters` excluded) is regression-guarded by three `#[cfg(feature = "hip")]` slow-tier suites: the two per-kernel suites `tests/gpu_ldpc_byte_identity.rs` (BP hard-decision codeword bit-for-bit, issue `a930be7f`) and `tests/gpu_demap_byte_identity.rs` (max-log demap within the measured ULP-or-absolute tolerance, issue `d3f1616a`), plus the chain-level closer `tests/gpu_byte_identity.rs` (full DVB-T2 BICM verdict end-to-end over r1/2 16-QAM, r2/3 64-QAM, r3/4 16-QAM at 200 frames each, issue `14f59c2d`). The chain suite shares ONE AWGN noise realisation between paths (GPU AWGN is separately proven by `f6004add`), runs both demap + LDPC BP on the GPU vs CPU, holds BCH outer decode on CPU on both arms (no GPU BCH kernel), and runs at an Es/N0 **above** each config's QEF threshold so the BP converges — the regime in which the §11 verdict is robust (below threshold, non-converged garbage codewords drift by the ULP residual and are outside the contract's scope). All three suites log `mean_iters` and never assert it (the §11 CPU-vs-GPU exclusion); `ber` is excluded entirely (status-quo amendment `152388f4`). Each GPU suite is gated on GPU presence (skips when `gf2_kernels_hip::host::device_mem_info().is_err()`) and carries `#[ignore]` per the test-tier rules; the chain-level three-column byte-identity attestation and per-config `mean_iters` are recorded in `dev/benchmarks/gf2-sim/gpu-stages-receipts.md`.
