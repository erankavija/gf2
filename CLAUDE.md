...
# Plotter: dev/benchmarks/dvb_t2_awgn/plot.py (LEGACY — see dev/benchmarks/gf2-sim/ for the v2 pipeline receipts)
...
# NOTE: dev/benchmarks/dvb_t2_awgn/ is the LEGACY benchmark directory (pre-gf2-sim pipeline).
# The v2 gf2-sim pipeline benchmarks, throughput receipts, and regression receipts live in
# dev/benchmarks/gf2-sim/ (see parallelism-receipts.md, cpu-foundation-receipts.md,
# gpu-stages-receipts.md, and dvb-t2-regression-receipts.md).
...
- **`HipStreamPool`** (`host/streams.rs`) — fixed-size pool of RAII `HipStream`s bound to one device; hands out streams by fixed index (`get`, deterministic ownership), round-robin (`acquire`, call-order cursor), or oldest-idle (`acquire_idle`, which surfaces genuine `hipStreamQuery` faults as `Err` while skipping merely-busy `hipErrorNotReady` streams). The pool is `Send + Sync` so it can be shared by reference (`&HipStreamPool`) across rayon workers (Phase C scheduler `75c22fa8`): worker `i` calls `pool.get(i % pool.len())` so it deterministically OWNS that stream — `acquire`'s shared atomic cursor advances in *call* order, which under a parallel iterator is scheduler-dependent and would desynchronise the recorded `stream_id` from the stream actually used. `Sync` is sound because `HipStream` is itself `Sync` (its `&self` methods are read-only or thread-safe HIP-runtime calls; no `&self` host mutation). A compile-time `_assert_sync` test in `streams.rs` enforces the bound.
...
| `ldpc/` | Belief-propagation decoder; `dvb_t2/` has tables from ETSI EN 302 755; `encoding/` uses Richardson-Urbanke with cache; `dvb_t2/concat.rs` = `DvbT2Concat` (BCH+LDPC concatenated codec); `dvb_t2/bit_interleaver.rs` = `DvbT2BitInterleaver` (column-row bit interleaver); `nr_5g/` = 5G NR LDPC per 3GPP TS 38.212 (BG1 46x68 + BG2 42x52 with per-`i_LS` shift tables for all 8 lifting sets, `QuasiCyclicLdpc::nr_5g` mother-code constructor, `nr_5g_rate_matched` 3GPP rate matching via LLR initialization, `shift_table` raw-table accessor, `max_payload_for_lifting` exact-Z payload helper, `interleaver.rs` = the §5.4.2.2 rate-matching bit interleaver validated against the spec text + Sionna's `generate_out_int`) — tables validated bit-exact against the external Sionna reference CSVs committed in `data/ldpc/nr_5g/` (`tests/nr5g_external_vectors.rs`, provenance in `data/ldpc/nr_5g/PROVENANCE.md`) |
...
| `presets/nr_5g` | `Pipeline::nr_5g()` typestate fluent builder (`BaseGraph`/`Nr5gRate`/`NrModulation`/`Nr5gDecoderConfig`/`Channel`, `NeedsBaseGraph→…→Ready`); the 5G NR LDPC preset over the graph API — `E` derived in the §5.4.2.1 floor form (a `Q_m` multiple by construction), exact-Z realization via `max_payload_for_lifting`; driven per-batch via `TopologyExecutor::run` (there is no NR sweep-level `Pipeline::run`; see the 5G NR preset section below) |
| `stage` | `Stage<I,O>` / `AnyStage` type-erasure (`erase`), `BatchSize`, `ExecutionClass`, `TypedBatch`, `FallbackKind` |
| `stages` | DVB-T2 BICM stage wrappers (`DvbT2Encode`, `BitInterleave`/`BitDeinterleave`, `GrayQamMap`/`GrayQamDemap`, decode, `DvbT2BchTail` — the BCH outer-decode tail paired with the GPU LDPC stage under `with_gpu(true)`) + `dvb_t2_bicm_stages` factory; `stages/nr_5g.rs` = the 5G NR wrappers (`Nr5gEncode`/`Nr5gDecode` over the rate-matched code, `Nr5gBitInterleave`/`Nr5gLlrDeinterleave` = §5.4.2.2, `NrGrayQamMap`/`NrGrayQamDemap` for `Q_m` ∈ {2,4,6,8}) |
...
| `executor` | Hybrid CPU/GPU executor: `Scheduler` (rayon-worker ∥ HIP-stream double-buffered CPU-prep/GPU-decode overlap, `75c22fa8`), `SimulationResults`/`SnrPointResult` (the `WorkerCounters` projection `Pipeline::run` returns), `OverlapTimeline` (overlap attestation), `RunPlan`, and `TopologyExecutor`/`DagOutputs` (`de160fc5`: per-stage-driven DAG execution — topo-ordered Kahn waves, fan-in/fan-out with intermediate-buffer refcounting, `execution_class()` routing with six-field per-stage `pipeline_stage` spans, panic-free execution-start lineage validation via `BuildError::ExecutionValidation`, and the stage-driven DVB-T2 sweep `run_dvb_t2_snr_point` whose chain-vs-SSOT byte-identity is guarded by `tests/stage_driven_byte_identity.rs`). `42eac5cc` landed OOM-catch / CPU-fallback dispatch (`dispatch_with_fallback`, `executor/failure.rs`); `571c11c4` landed GPU drain-for-checkpoint + the checkpointed hybrid sweep (`executor::drain`, `Pipeline::run_checkpointed`). Failure-policy refinements: `strict_gpu` promotes OOM (and only OOM) to fatal; hard-fails write a JSON diagnostic dump to `PipelineConfig::diagnostic_dump_dir`; CPU fallbacks with stateful scratch (e.g. the RNG-bearing `Awgn`) are REFUSED with a typed error rather than silently default-seeded (§11 guard); checkpointed sweeps abort resumably on a recoverable GPU fault instead of substituting the fallback (checkpoint determinism). `bb11c2e6` factored the uncheckpointed scheduler loop and the checkpointed drain loop onto ONE shared double-buffer core (`executor/hybrid_core.rs`, `feature = "hip"`), parameterized by a `BatchHooks` trait (monomorphized, no `dyn` in the per-batch path) whose two hook divergences are the failure semantics — the scheduler substitutes the CPU fallback (`dispatch_with_fallback`), the checkpointed sweep PROPAGATES the recoverable fault (OPTION (a), abort-resumably, the last committed heartbeat checkpoint surviving; guarded by `tests/hybrid_resume.rs::hybrid_checkpointed_recoverable_fault_aborts_resumably`) — and stop-after-batch (the checkpointed loop stops at batch-aligned interrupt points). The shared core also gives the checkpointed loop `pipeline_stage` span parity with the scheduler (the `OverlapTimeline` interval sink is the uncheckpointed scheduler's; the checkpointed path passes none). OPTION (b) (deterministic fallback-in-checkpointed-sweep) is intentionally NOT implemented: `inject_gpu_oom_modulus` is result-affecting on the fallback path yet excluded from `config_hash`, so it needs a lead/user decision |
