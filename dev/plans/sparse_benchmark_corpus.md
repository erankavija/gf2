...
| `sparse-matmul × GF(2)` | **EXCLUDED:`schema-violation`:no public sparse-sparse matmul in candidate matrix.** | n/a | fflas-ffpack `fflas_sparse` lacks `fspmm` (sparse × sparse → sparse); only `fspmv` and `fspmm-dense` (sparse × dense). LinBox has `SparseMatrixDom::mul` but it materialises the dense product, which is the `sparse×dense` row, not sparse-sparse. M4RI `mzd_sparse_mul` does not exist. **User-approval flag in § 6** to confirm exclusion is acceptable; the alternative is a self-reference-only row using gf2-core's `SpBitMatrix::matmul` (currently unimplemented) — a no-op bench until that lands. |
| `sparse-matmul × GF(p)` | **EXCLUDED:`schema-violation`:no public sparse-sparse matmul.** | n/a | Same as above. fflas-ffpack `fspmm` is sparse × dense; FLINT `nmod_mat_mul` is dense × dense; LinBox's `SparseMatrixDom::mul` materialises a dense intermediate. **User-approval flag in § 6.** |
| `sparse-matmul × GF(2^m)` | **EXCLUDED:`schema-violation`:no public sparse-sparse matmul.** | n/a | Same as above. **User-approval flag in § 6.** |
...
| `sparse-elim × GF(2^m)` | **EXCLUDED:`semantics-mismatch`:no fflas-ffpack/LinBox sparse-elim path covers GF(2^m) at the field level.** Falls back to **gf2-core self-reference if implemented; otherwise the cell is not benched.** | n/a — `gf2-core` `SparseFieldMatrix<Gf2mWide<…>>` does not currently expose an `rref` method (sparse Gauss-Jordan over arbitrary fields is not implemented). | Same GivaroExtension-cost rationale as the GF(2^m) `spmv` row. **User-approval flag in § 6.** |
...
2. **`sparse-matmul × {GF(2), GF(p), GF(2^m)}` — class `schema-violation`.** No public sparse × sparse matmul exists in fflas-ffpack, LinBox, M4RI/M4RIE, NTL, or FLINT — every candidate either materialises a dense intermediate (which is the `sparse×dense` row) or lacks the symbol entirely. The cell is excluded from the reference matrix; gf2-core's `SpBitMatrix::matmul` (currently unimplemented) is filed as a follow-up under epic `97bf0879`. Approval requested for the exclusion.
...
4. **`sparse-elim × GF(2^m)` — class `semantics-mismatch`.** Same field-level gap; LinBox's `Method::SparseElimination` over GivaroExtension is the only candidate and is not performance-comparable. The cell is currently not benched at all because gf2-core does not expose `SparseFieldMatrix<Gf2mWide<…>>::rref`. Approval requested for `(EXCLUDED + not-benched)`; the alternative is to file `gf2-core sparse-elim over GF(2^m)` as new gf2-core work, which extends `97bf0879`'s scope.
