...
| `spmv × GF(2^m)` | **gf2-core self-reference (canonical).** | gf2-core's `SparseFieldMatrix<Gf2mWide<…>>::matvec` against the dense round-trip equivalent. Cross-library equality is not asserted. | fflas-ffpack `fflas_sparse` is `Field`-templated but its specialisations are `Modular<float|double|int*>`-only; GF(2^m) in fflas-ffpack rides through `Modular<float>` over a polynomial-coefficient lift, which is not a fair comparison. LinBox's `SparseMatrix<GivaroExtension<…>>` exists but the GivaroExtension polynomial multiplication path is vastly slower than gf2-core's PCLMULQDQ-backed `Gf2mWide` and would dominate the timing — see Wave-2 M4RIE evidence for the same pattern. M4RIE has no public sparse type. Per the 2026-05-04 user decision (see § 9), the cell is self-reference rather than excluded. |
...
| `sparse×dense × GF(2^m)` | **gf2-core self-reference (canonical).** | gf2-core's `SparseFieldMatrix<Gf2mWide<…>>::matmat` against the dense round-trip equivalent. | Same field-level gap as `spmv × GF(2^m)` — fflas-ffpack and LinBox sparse over GF(2^m) ride GivaroExtension polynomial mult and are not performance-comparable. Per the 2026-05-04 user decision (see § 9), the cell is self-reference rather than excluded. The existing `SparseFieldMatrix::matmat` provides the canonical numbers; no new gf2-core impl is required for this cell beyond what `5a9a287` already landed. |
...
## 6. Exclusion proposals — original drafts (all resolved as of 2026-05-04)

These were drafted as proposals requiring user approval before `47698404` dispatched. All six were resolved during Wave-3 closure (2026-05-04T11:42Z user escalation + protocol Amendment 2 landing); the resolutions are recorded inline below. The historical-draft form is kept so the audit trail explains what `47698404` will and will not see.

1. **`spmv × GF(2^m)` — *resolved 2026-05-04*.** Original proposal: class `semantics-mismatch`, fall back to gf2-core self-reference. Per the same user decision that resolved proposals #2–#4, the cell's accepted-reference is gf2-core's `SparseFieldMatrix<Gf2mWide<…>>::matvec` self-reference (no external library is performance-comparable; fflas-ffpack / LinBox sparse over GF(2^m) ride GivaroExtension polynomial multiplication and would be ≥10× slower than gf2-core's PCLMULQDQ-backed `Gf2mWide`). The cell is no longer "approval-pending" — it is accepted as a self-reference cell. Listed in § 4 row `spmv × GF(2^m)` as the canonical reference.
...
3. **`sparse×dense × GF(2^m)` — *resolved 2026-05-04*.** Original proposal: class `semantics-mismatch`, fall back to gf2-core self-reference. Resolved by the same user decision as proposal #1 (same field-level gap, same self-reference fallback). The accepted reference is gf2-core's `SparseFieldMatrix<Gf2mWide<…>>::matmat`. The cell is no longer "approval-pending".
...
5. **SuiteSparse Matrix Collection — *resolved 2026-05-04* via user decision (non-adoption).** The collection is rich but its pinning model does not fit the existing `image.lock` schema, and committing canonical `.mtx` files under `benchmarks/sparse-corpus/` would create a parallel pin mechanism. Confirmed by user as a scope decision; SuiteSparse is not used.

6. **Protocol § 7 CSV-schema operation set extension — *landed 2026-05-04* via Amendment 2 to the protocol.** The original proposal here was that `47698404` would land a protocol § 7 amendment to add `sparse-matmul`, `sparse×dense`, and `sparse-elim` to the allowed `operation` values. After user approval, the amendment landed inside this design's session as protocol § 14 *Amendment 2* (`dev/plans/sota_reference_acceptance_protocol.md` § 14 + § 7 list update). The `analyze.py` validator update is owned by `47698404` (not yet dispatched). This `47698404`-side update is the only remaining piece; the protocol-side amendment is closed.

**Final cell-count tally.** After resolution: 0 protocol-class exclusion cells in the 12-cell operations × field matrix. All 12 cells carry hard references — either external (fflas-ffpack canonical + LinBox cross-check for GF(p) cells; LinBox canonical for sparse-elim × {GF(2), GF(p)}) or gf2-core self-reference for the GF(2^m) cells where no external library is performance-comparable.

## 7. Open questions — for the lead

1. **Whether to preemptively file `gf2-core SpBitMatrix::matmul` and `SparseFieldMatrix::rref` as sparse-impl follow-ups — *resolved 2026-05-04*.** Per the user's Wave-3 closure decision (see § 9 below), both were filed and have landed: `2403c054` (`SpBitMatrix::matmul`, GF(2)) and `eb57f944` (`SparseFieldMatrix::matmul` + `::rref`, generic over `F: FiniteField`). The previously-excluded sparse-matmul and sparse-elim × GF(2^m) cells are now self-reference cells with the canonical gf2-core path.
...
Both `[hard]` criteria are self-satisfied **in this document**; nothing in the *design* is deferred to a downstream artefact. The `47698404` issue consumes this design without re-litigating either the corpus or the reference set; it inherits the impl-task dependencies on `2403c054` and `eb57f944` (wired in JIT 2026-05-04). The `47698404`-side validator update (`benchmarks/analyze.py::OPERATION_ORDER` was extended in this commit; downstream `47698404` will add validator-side schema checks once the harness is written) is out of scope for this design issue.
