...
6. **`Fp<P>`'s Proth detection is const but the prime is a const-generic.** `TWO_ADICITY` in Task 6 needs to be derivable from `P` at compile time. If `const fn` in trait-const position hits a nightly-only path on stable Rust, fall back to a `const` on a helper `struct ProthConst<const P: u64>` and document the workaround.
