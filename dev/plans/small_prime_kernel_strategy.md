...
2. **Throughput envelope.** AVX2 hits 16 × 16-bit MACs per cycle via `_mm256_madd_epi16` (1-cycle latency on Zen-3 per Agner Fog's instruction tables), so the upper bound on a perfectly-scheduled inner kernel at 5 GHz is $16 \cdot 5 \times 10^9 = 8 \times 10^{10}$ MACs/s — 80 Gop/s in the bench's `2 m k n` op-count metric. fflas-ffpack hits 50 Gop/s for GF(7) and 128 Gop/s for GF(251) at $n = 256^3$; the AVX2-MAC envelope is sufficient to match GF(7) (50 / 80 = 62 %) and reach about 60 % of the GF(251) cell. The 1.5x criterion at GF(251) becomes 128 / 1.5 = 85.3 Gop/s; reaching 85.3 / 80 = 107 % of the AVX2-MAC peak is implausible without AVX-512, but the criterion of `662f7a15` says "within 1.5x of fflas **or faster**" — so the criterion at GF(251) is met if gf2-core hits at least $128 / 1.5 \approx 85\,\text{Gop/s}$. Reaching 85 % of the AVX2-MAC peak (= 68 Gop/s) is **insufficient** to clear 1.5x at GF(251).

   This is a **soft criterion-risk**: the 1.5x target at GF(251) is on the edge of the AVX2-MAC envelope. § 8 risk #3 records this and § 7 step 7 instructs `662f7a15` to measure GF(251) before GF(7) and GF(31), so any short-fall surfaces early. The mitigation if the GF(251) target is unreachable on AVX2 is to escalate to the user per the `[hard]` amendment policy in `CLAUDE.md` § *Success-criterion maturity markers* — not to abandon the design. (For GF(7) and GF(31), the 1.5x targets are 50 / 1.5 ≈ 33.5 Gop/s, well within the envelope.)
...
1. **GF(251) AVX2 envelope (medium).** § 6 #2 records that 1.5x of fflas's GF(251) cell is 85.3 Gop/s, while the AVX2 16-bit-MAC peak at 5 GHz is 80 Gop/s — GF(251) sits on the edge of the achievable envelope. Mitigation: run GF(251) first per § 7 step 7. If short-fall, escalate the criterion to the user per the `[hard]` amendment policy with the measured number; do not silently regrade.
