...
| One strategy is selected with feasibility evidence. | **MET** — § 6 selects exactly one strategy (Candidate C — SIMD-lane AVX2 byte/word kernel) with four explicit pieces of feasibility evidence (architectural reuse, throughput envelope, no new external dependency, feasibility within `662f7a15`'s scope). § 6 #2 quantifies the throughput envelope per prime against the now-amended `662f7a15` per-prime targets: GF(7) and GF(31) at 42 % of AVX2-MAC peak (ample headroom under the `[hard]` 1.5× absolute targets); GF(251) under the `[aspirational]` envelope-bound target with re-escalation threshold ~3.2× of fflas. The amendment to `662f7a15`'s GF(251) `[hard]` → `[aspirational]` was recorded at this design's authoring time per `CLAUDE.md` § *Success-criterion maturity markers* with the 80 Gop/s AVX2-MAC peak as the empirical falsifier of the 85.3 Gop/s 1.5× target. § 7 provides the file-level implementation outline including the GF(251)-first measurement order. | § 6, § 7. |
