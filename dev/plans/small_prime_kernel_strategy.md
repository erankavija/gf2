...
> Added 2026-05-06 per § 4.5, § 5.5, § 6.1 amendment. Lists the file-level changes the `662f7a15` rework will land. Note on baseline state: as of this commit, `main` does **not** yet contain the Wave-6B Candidate C source code — the C implementation (`fp_small.rs`, `x86/fp_small.rs`, `maybe_fp_small()`, the `if P <= 251` branch in `try_simd_gemm_classical`) lives on the worker branch `worktree-agent-662f7a15` only, awaiting rework against this design. The 662f7a15 rework will land both the C baseline files (preserved from the worker branch) AND the new Candidate F files in a single integration: C remains as the AVX2-only-no-FMA3 runtime fallback per § 6.1, F is the FMA3-host primary path. The downstream implementer should rebase `worktree-agent-662f7a15` onto `main` HEAD, layer the Candidate F additions described below on top, and integrate as a single rework commit per the lead's cherry-pick protocol.
...
- `[E14]` Project memory file `~/.claude/projects/-home-vkaskivuo-Projects-gf2/memory/feedback_hard_criterion_self_satisfaction.md` (outside the repo tree, indexed via `~/.claude/projects/-home-vkaskivuo-Projects-gf2/memory/MEMORY.md`) — the self-satisfy-IN-doc convention used in § 9. The principle: when a `[hard]` criterion names a downstream artefact, the design making the designation IS the evidence; deferring to the consumer issue is an automatic review FAIL.
