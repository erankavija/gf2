...
5. **Add the dispatch branch in `crates/gf2-core/src/field/matrix.rs::try_simd_gemm_classical`.** Insert the new branch **above** the existing Candidate C `if F::PRIME <= 251` branch:
   ...
   ```
   The pure-`const` selector `select_f32_path::<F>(_m, _k, _n) -> bool` encodes the uniform dispatch rule from § 6.1: it returns `F::PRIME <= 251` for every $(m, k, n)$. The $(m, k, n)$ parameters are accepted for forward-compatibility with a later per-cell refinement, but are unused by the current rule. The selector is `const fn` so it is constant-folded for the call sites where `F::PRIME` is known statically. The runtime decision between F and C is made entirely by `maybe_fp_small_f32()` returning `None` (which happens iff FMA3 is unavailable at runtime); the selector itself does not pick C over F at any cell.
