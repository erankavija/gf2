...
- `matmul × GF(2)` reaches only `0.13×` of M4RI at `n=1024` (`387 Gops/s` vs `3.02 Tops/s`) and `0.17×` at `n=4096`. The uncompetitiveness profile captured the pre-`59c487c3` state: M4RI switched to Strassen-Winograd at large sizes while gf2 used pure M4RM. Post-`59c487c3`, gf2 has a conservative Strassen-family layer, but `dev/bench_results/2026-04-29-strassen-matmul-crossover.md` keeps production dispatch on M4RM through the measured sizes and only enables square power-of-two Strassen at `n >= 16_384`.
