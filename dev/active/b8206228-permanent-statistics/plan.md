...
| Retry after a halt is predeclared | Rerunning a halted cell until it passes spends the error budget silently. The protocol fixes what may be rerun, how many attempts are permitted, and how a rerun charges the budget; the driver refuses a rerun outside that rule, and halted cells stay in the dataset with their verdicts. |
