//! F_7 packed-encoding research prototype (JIT issue f10152f6, R2).
//!
//! Prototypes 4 candidate encodings for $\mathbb{F}_7$ vectors and measures
//! per-element wall-clock time for `add`, `sub`, `mul`, `div`. Mirrors the
//! shape of `dev/research/f5_packing/`. Not part of the gf2 workspace.
//!
//! Build:   `cargo build --release`
//! Test:    `cargo test --release`     (correctness vs `(a OP b) % 7`)
...
use common::{bench_op_ns_per_elem, F7Encoding, Lcg, N_BENCH, REPEATS};

/// Run all four ops on a single encoding and return `(add, sub, mul, div)`
/// median ns/element.
fn bench_one<E: F7Encoding + Clone>(a_vec: &[u8], b_vec: &[u8], b_nonzero: &[u8]) -> [f64; 4] {
    let n = a_vec.len();
    let a = E::pack(a_vec);
    let b = E::pack(b_vec);
...
fn print_row(name: &str, ns: [f64; 4]) {
    println!(
        "  {:<44}  add={:>7.2}  sub={:>7.2}  mul={:>7.2}  div={:>7.2}",
        name, ns[0], ns[1], ns[2], ns[3]
    );
}

fn main() {
    println!("F_7 packed-encoding prototype — JIT f10152f6 (R2)");
    println!(
    ...
    let mut rng = Lcg::new(0xF7_DEADBEEF);
    let a_vec = rng.f7_vec(N_BENCH);
    let b_vec = rng.f7_vec(N_BENCH);
    let b_nz = rng.f7_vec_nonzero(N_BENCH);
    ...
    println!("dev/plans/r2_f7_encoding_decision.md for the chosen winner.");
}
