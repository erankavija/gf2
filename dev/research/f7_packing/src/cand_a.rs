...
use crate::common::{ref_add, ref_mul, ref_sub, F7Encoding, INV_F7};
...
/// Packed F_7 vector for Candidate A.
#[derive(Clone, Debug)]
pub struct VecA {
    words: Vec<u64>,
    len: usize,
...
mod tests {
    ...
    fn exhaustive_add_pairs() {
        for a in 0u8..7 {
            for b in 0u8..7 {
                ...
                let vb = VecA::pack(&[b]);
                va.add_assign(&vb);
                assert_eq!(va.unpack()[0], (a + b) % 7, "add {a}+{b}");
            }
        }
    }
    ...
    fn exhaustive_sub_pairs() {
        for a in 0u8..7 {
            for b in 0u8..7 {
                ...
                let vb = VecA::pack(&[b]);
                va.sub_assign(&vb);
                assert_eq!(va.unpack()[0], (a + 7 - b) % 7, "sub {a}-{b}");
            }
        }
    }
    ...
    fn exhaustive_mul_pairs() {
        for a in 0u8..7 {
            for b in 0u8..7 {
                ...
                let vb = VecA::pack(&[b]);
                va.mul_assign(&vb);
                assert_eq!(va.unpack()[0], (a * b) % 7, "mul {a}*{b}");
            }
        }
    }
    ...
    proptest! {
        ...
        fn prop_add_matches_scalar(
            ...
            let got = va.unpack();
            for i in 0..n {
                prop_assert_eq!(got[i], (a[i] + b[i]) % 7);
            }
        }
        ...
        fn prop_sub_matches_scalar(
            ...
            let got = va.unpack();
            for i in 0..n {
                prop_assert_eq!(got[i], (a[i] + 7 - b[i]) % 7);
            }
        }
