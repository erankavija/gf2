...
- **Closure status of measured/self-canonical cells (per authoritative parity evidence docs):**
  ...
  - **SC#3+SC#4 status (2026-05-09):** Annex A8 routes all 76 FAIL cells to named JIT issues (57 → `615db3b9`, 3 → `52cce970`, 2 → `974a85bd`, 3 → `aaa847cf`, 10 → `5ce13bae`, 1 → already in `615db3b9`). Annex A9 (PENDING umbrella) is **SUPERSEDED 2026-05-09** — the cells it previously covered are now either measured (GF(31) bucket via `[EX]`) or classified EXCLUDED [§ 6.3] (GF(2) pluq/solve and matmul × GF(2^4) — see § 6.3). Combined with A1–A7, every in-scope cell has PASS, AMENDED, FAIL→A8-routed, or EXCLUDED status. **Zero uncovered cells. SC#3+SC#4 conjunction satisfied.**
...
| `[EX]` | `dev/bench_results/2026-05-08-pending-cell-measurement.md` | Wave-12 bench extension (**COMPLETE 2026-05-09**): direct Criterion measurements for all originally PENDING GF(31) cells. Host: AMD Ryzen 9 5900X Zen 3, 2026-05-08/09. Benches: `fieldmatrix_ple`, `fieldmatrix_solve` (extended with `Fp<31>`), and `charpoly` (extended with `Fp<31>` in `bench_minpoly_reference_sweep` + `bench_charpoly_reference_sweep`). Covers all 20 GF(31) cells: 16 dense-LA (pluq, echelon, invert uniform+deficient, solve uniform+deficient at n=64,256) + 4 charpoly/minpoly at n=64,256. Net: 14 PASS, 6 FAIL routed to A8 rows 71–76. |
