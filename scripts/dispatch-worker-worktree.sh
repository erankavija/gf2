...
#
# Replaces Agent's `isolation: "worktree"` parameter which was
# observed in babcf05e session 6 to branch from a stale ancestor
# (41 commits behind main) — see dev/active/babcf05e-handoff-4.md
# TRAP 1 and ~/.claude/skills/project-lead/references/worktree-dispatch-protocol.md.
#
# Usage:
...
Hard rules for path discipline (TRAP 6 from babcf05e session 6):
...
echo
echo "Post-completion: run scripts/check-leak-into-main.sh to verify"
echo "main's working tree did not absorb any worker output."
