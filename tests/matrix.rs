...
fn test_new_zero_basic() {
    ...
    for r in 0..3 {
        for c in 0..4 {
            assert_eq!(m.get(r, c), false, "bit at ({}, {}) should be false", r, c);
        }
    }
}
...
fn test_get_set_large() {
    ...
    assert_eq!(m.get(5, 63), true);
    assert_eq!(m.get(5, 64), true);
    assert_eq!(m.get(5, 127), true);
    assert_eq!(m.get(5, 62), false);
    assert_eq!(m.get(5, 65), false);
}
...
fn test_identity_square() {
    ...
    // Check diagonal is 1
    for i in 0..4 {
        assert_eq!(m.get(i, i), true, "diagonal ({}, {}) should be true", i, i);
    }
    ...
    for r in 0..4 {
        for c in 0..4 {
            if r != c {
                assert_eq!(m.get(r, c), false, "off-diagonal ({}, {}) should be false", r, c);
            }
        }
    }
}
...
fn test_row_words_mut() {
    ...
    assert_eq!(m.get(0, 0), true);
    assert_eq!(m.get(0, 63), true);
    assert_eq!(m.get(0, 64), true);
    assert_eq!(m.get(0, 71), true);
    assert_eq!(m.get(0, 72), false);
}
...
fn test_transpose_rectangular() {
    ...
    // Check transpose
    assert_eq!(mt.get(0, 0), true);
    assert_eq!(mt.get(2, 0), true);
    assert_eq!(mt.get(1, 1), true);
    assert_eq!(mt.get(0, 1), false);
    assert_eq!(mt.get(1, 0), false);
}
